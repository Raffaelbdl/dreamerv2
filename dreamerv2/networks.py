...
min_denom = 0.000001
activation_dict = {"silu": jax.nn.silu, "elu": jax.nn.elu}
...
class reward_network(hk.Module):
    ...
    def __call__(self, phi, h, key=None):
        ...
        if self.learn_reward_variance:
            sigma = jax.nn.softplus(hk.Linear(1)(x))[0]
...
class next_phi_network(hk.Module):
    ...
    def __call__(self, h, key):
        ...
        if self.latent_type == "gaussian":
            ...
            sigma = jax.nn.softplus(hk.Linear(self.num_features)(jnp.ravel(x)))
            sigma = jnp.clip(sigma, min_denom, None)

            x = mu + sigma * jax.random.normal(key, mu.shape)
            ...
            x = jax.nn.tanh(mu + sigma * jax.random.normal(key, mu.shape))
            return x, {"mu": mu, "sigma": sigma}
        elif self.latent_type == "categorical":
            logits = jnp.reshape(
                hk.Linear(self.num_features * self.feature_width)(jnp.ravel(x)),
                [self.num_features, self.feature_width],
            )
            ...
            x = jax.nn.one_hot(
                jax.random.categorical(key, logits), self.feature_width, axis=1
            )
            x = probs + jax.lax.stop_gradient(x - probs)
    ...
    def __call__(self, s, h, key):
        ...
        for i in range(self.conv_depth):
            x = self.activation_function(
                hk.Conv2D(self.num_conv_filters * (2**i), 3, padding="VALID")(x)
            )
...
class state_conv_network(hk.Module):
    ...
    def __call__(self, phi, h):
        desired_shape = [
            self.state_shape[0] - 2 * self.conv_depth,
            self.state_shape[1] - 2 * self.conv_depth,
            self.num_conv_filters * (2 ** (self.conv_depth - 1)),
        ]
        num_units = 1
        ...
        for i in range(self.conv_depth - 1):
            x = self.activation_function(
                hk.Conv2DTranspose(
                    self.num_conv_filters * 2 ** (self.conv_depth - i - 1),
                    3,
                    padding="VALID",
                )(x)
            )
        ...
        if self.binary:
            logit = hk.Conv2DTranspose(
                self.state_shape[2],
                3,
                output_shape=self.state_shape[:2],
                padding="VALID",
            )(x)
            # Note this returns the logit of S, we wish to apply a sigmoid after to keep it bounded
            return {"logit": logit}
        else:
            mu = hk.Conv2DTranspose(
                self.state_shape[2],
                3,
                output_shape=self.state_shape[:2],
                padding="VALID",
            )(x)
            log_sigma = hk.Conv2DTranspose(
                self.state_shape[2],
                3,
                output_shape=self.state_shape[:2],
                padding="VALID",
...
class state_flat_network(hk.Module):
    ...
    def __call__(self, phi, h):
        ...
        if self.binary:
            ...
            sigma = jax.nn.softplus(hk.Linear(self.state_width)(x))
            sigma = jnp.clip(sigma, min_denom, None)
            return {"mu": mu, "sigma": sigma}
